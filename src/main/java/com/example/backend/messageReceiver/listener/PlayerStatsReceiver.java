...
public class PlayerStatsReceiver {
    ...
    public void receiveStatsMessage(Map<String, Object> playerData) {
        try {
            String firstName = (String) playerData.get("firstName");
            String lastName = (String) playerData.get("lastName");
            ...
            if (playerOptional.isPresent()) {
                ...
                for (Map<String, Object> stats : seasonStats) {
                    String season = (String) stats.get("season");

                    // Usuń istniejące statystyki z tego sezonu, jeśli istnieją
                    Optional<PlayerSeasonStats> existingStats = playerSeasonStatsRepository.findByPlayerAndSeason(player, season);
                    if (existingStats.isPresent()) {
                        playerSeasonStatsRepository.delete(existingStats.get());
                        System.out.println("Nadpisano statystyki dla sezonu: " + season + " zawodnika: " + firstName + " " + lastName);
                    }
            ...
            System.err.println("Error processing player data: " + e.getMessage());
            e.printStackTrace();
        }
    }
